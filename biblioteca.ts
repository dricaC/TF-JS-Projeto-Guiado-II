class Biblioteca{
  //livros: Array <Livro> =[];   outra forma de declara um array

  //private renderizarEmprestimosAtivos(): void{ // verificar
    livros: Livro [] = []; // lista vazia
    alunos: Aluno [] = [];
  ...
  // --------- [Não mexer] Responsaveis por renderizar no html
    ...
    encontrarLivro(id:number): Livro{
      ...
      //const livroEncontrado = this.livros.find((livro) => livro.id === id) as livro;  // as tipo livro, comparativo
    ...
    adicionarAluno (aluno: Aluno) {
      ...
      //Não precisa renderizar, pois não aparecer na tela
    ...
    //verificando se a mtricula existe
      if (!aluno.matricula){
      ...
      if (!livro.estaDisponivel){
        alert (`O livro" ${livro.titulo} "não está disponível.`);
        return false;
      }
    ...
    livro.emprestarLivro();
      ...
      this.emprestimos.push(emprestimos);
